...
class TelegramAdapter(BasePlatformAdapter):
    ...
    async def _handle_callback_query(self, update: "Update", context: "ContextTypes.DEFAULT_TYPE") -> None:
        ...
        for prefix, handler in (
            ...
            (("accept:", "reject:", "rate:"), self._handle_paper_feedback_callback)):  # local: paper-digest
            if data.startswith(prefix):
                await handler(query, data, cb)
                return

    async def _handle_paper_feedback_callback(self, query, data: str, cb: Dict[str, Any]) -> None:
        """Handle paper-digest feedback callbacks (thumbs up/down ratings). Local patch."""
        try:
