...
#
# The honcho memory provider's SDK (honcho-ai) IS baked in (local patch;
# upstream excludes it): our local agents run memory.provider=honcho, and
# relying on the first-use lazy install into /opt/data/lazy-packages means a
# fresh profile silently loses Honcho until something triggers
# ensure("memory.honcho") — `hermes honcho status` just reports
# "honcho-ai is not installed".
#
...
RUN touch ./README.md
RUN uv sync --frozen --no-install-project --extra all --extra messaging --extra otlp --extra anthropic --extra bedrock --extra azure-identity --extra matrix --extra google-chat --extra honcho
